...
# THIS IS A FAKE UNSTRUCTURED...
# IT ACTUALLY IS HAR3 CODE WHERE I JUST ALLOW NEGATIVE RHO
# CAN ONLY HANDLE 2X2 FOR NOW...
...
run_up_mcar_unstr <- function(
  ...
  for (it in (oldT + 1):Tn) {
    ...
    ## candidate
    u <- (1 + rho[it - 1]) / 2
    u_star <- rbeta(1, qr * u + 1, qr * (1 - u) + 1)
    rho_star <- 2 * u_star - 1
    ...
    if (prod(info_cand < thres) == 1) { # then MH
      ...
      r_beta1 <- dbeta(u_star, a_r, b_r, log = TRUE) - dbeta(u, a_r, b_r, log = TRUE)
      r_beta2 <- dbeta(u, qr * u_star + 1, qr * (1 - u_star) + 1, log = TRUE) -
        dbeta(u_star, qr * u + 1, qr * (1 - u) + 1, log = TRUE)
